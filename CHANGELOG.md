...
| Feature | Complete plate_core side of E2E visual evidence tracking (#64): flexible detection heuristic for playwright.config.* (any ext) + optional dir/dep per spec; `gh plate features --local` for fs-based verification; updated plate agent prompt with E2E tool awareness and #64/#133 linkage. Closes #64. | #64 | This PR | `src/plate_core/features.py`, `src/plate_core/cli.py`, `plugin/agents/plate.agent.md`, `tests/test_features.py`, `CURRENT.md`, `CHANGELOG.md` |
| Feature | CLI-driven PR feedback babysitting: added `gh plate pr babysit <number>`, MCP tools `plate_pr_babysit` and `plate_resolve_review_thread`, updated plate agent babysitting workflow, and deprecated `plates-address-pr-feedback.yml` to manual-only fallback. Closes #112. | #112 | This PR | `src/plate_core/pr_babysit.py`, `src/plate_core/cli.py`, `src/plate_core/mcp_server.py`, `plugin/agents/plate.agent.md`, `.github/workflows/plates-address-pr-feedback.yml`, `tests/test_pr_babysit.py`, `AGENTS.md`, `CURRENT.md` |
